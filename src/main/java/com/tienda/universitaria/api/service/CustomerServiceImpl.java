...
public class CustomerServiceImpl implements CustomerService {
    ...
    public CustomerDtos.CustomerResponse create(CustomerDtos.CustomerCreateRequest req) {
        if (req == null) {
            throw new IllegalArgumentException("CustomerCreateRequest must not be null");
        }
        if (req.email() == null || req.email().isBlank()) {
            throw new IllegalArgumentException("email must not be blank");
        }
        if (customerRepository.existsByEmail(req.email())) {
            throw new IllegalArgumentException("Customer email already exists: " + req.email());
        }
    ...
    public CustomerDtos.CustomerResponse update(UUID id, CustomerDtos.CustomerUpdateRequest req) {
        ...
        if (req == null) {
            throw new IllegalArgumentException("CustomerUpdateRequest must not be null");
        }
        ...
        if (req.email() != null && !req.email().isBlank()) {
            ...
            if (!newEmail.equalsIgnoreCase(currentEmail) && customerRepository.existsByEmail(newEmail)) {
                throw new IllegalArgumentException("Customer email already exists: " + newEmail);
            }
        }
    ...
    public CustomerDtos.CustomerResponse getByEmail(String email) {
        ...
        Customer customer = customerRepository.findByEmail(email)
                .orElseThrow(() -> new EntityNotFoundException("Customer not found for email: " + email));
        return customerMapper.toResponse(customer);
    }
